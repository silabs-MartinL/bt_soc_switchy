...
static void app_tick_timer_cb(sl_sleeptimer_timer_handle_t *tick_timer_cb,
		void *data);
...
{ 0x24, 0x12, 0xb5, 0xcb, 0xd4, 0x60, 0x80, 0x0c, 0x15, 0xc3, 0x9b, 0xa9, 0xac,
		0x5a, 0x8a, 0xde };

// LED Control Characteristic UUID: 5b026510-4088-c297-46d8-be6c736a087a
const uint8_t charUUID[16] =
{ 0x7a, 0x08, 0x6a, 0x73, 0x6c, 0xbe, 0xd8, 0x46, 0x97, 0xc2, 0x88, 0x40, 0x10,
		0x65, 0x02, 0x5b };
// Strings
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
	...
	case sl_bt_evt_scanner_legacy_advertisement_report_id:
		...
		{
			app_log(
					"sl_bt_on_event(sl_bt_evt_scanner_legacy_advertisement_report_id)\n");
			// Open connection
	...
	case sl_bt_evt_connection_opened_id:
		...
		// Discover services
		sc = sl_bt_gatt_discover_primary_services_by_uuid(_conn_handle, 16,
				serviceUUID);
...
void sl_button_on_change(const sl_button_t *handle)
{
	...
	if (1 == btn_state)
	{
		...
		if (btn != 0xFF)
		{
			// Button 0 - attempt to turn on
			if (0 == btn)
			{
				(void) app_set_state(APP_STATE_OFF);
			}
			// Button 1 - attempt to turn off
			else if (1 == btn)
			{
				(void) app_set_state(APP_STATE_ON);
			}
		}
	}
}
...
sl_status_t app_set_state(uint8_t new_state)
{
	...
	if (SL_STATUS_OK == sc)
	{
		...
		if (APP_STATE_ON == state)
		{
			// Transmit data
			data = 1;
			sc_tx = sl_bt_gatt_write_characteristic_value(_conn_handle,
					_char_handle, 1, &data);
